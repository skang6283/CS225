...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->is_leaf) {//== true && subroot->elements[first_larger_idx] != pair){
        typename std::vector<DataPair>::iterator pos = subroot->elements.begin()+first_larger_idx;
        subroot->elements.insert(pos, pair);
