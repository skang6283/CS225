...
// You will start with a square grid (like graph paper) with the specified height and width. You will select 
// random walls to delete without creating a cycle, until there are no more walls that could be deleted without creating a cycle. 
...
// 	This uses your representation of the maze to determine whether it is possible to travel in the given direction 
...
// This function will be very helpful in solving the maze. It will also be used by the grading program to verify 
//	that your maze is a tree that occupies the whole grid, and to verify your maze solution. 
...
//	which is defined as the length (measured as a number of steps) of 
//	the only path through the maze from the origin to that square.

// Select the square in the bottom row with the largest distance from the 
//	origin as the destination of the maze. solveMaze() 
//	returns the winning path from the origin to the destination 
...
//except the entrance (1,0) through (9,0). For each square in the maze, 
//call its maze coordinates (x,y). If the right wall exists, 
...
// Start at pixel (5,5). 
//	Each direction in the solution vector corresponds to a trail 
//	of 11 red pixels in the given direction. If the first step is downward, 
