...
class BinaryTree
{
    ...
    private:
        ...
        int checkmin(Node * subRoot) const;
        int checkmax(Node * subRoot) const;
        void getPaths(std::vector<std::vector<T>>& paths,std::vector<T> &temp, Node* subRoot )const;
        int sumDistances(Node* subRoot, int incr) const;
