...
//    * for you.
    ...
    for(unsigned i = 0 ; i < frequencies.size() ; i++){
      TreeNode* tmp  = new TreeNode(frequencies[i]);
      singleQueue.push(tmp);
    }
...
//     * the back of the mergeQueue.
      while( (singleQueue.size() + mergeQueue.size() )  != 1 ){
          ...
          int newfreq = small1->freq.getFrequency() + small2->freq.getFrequency();
          TreeNode* internal = new TreeNode(newfreq);
