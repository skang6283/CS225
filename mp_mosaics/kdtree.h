...
class KDTree
{
  ...
  private:
    ...
    int partition(vector<Point<Dim>> & list ,int left, int right, size_t pivotIndex, int curDim);
    Point<Dim> select(vector<Point<Dim>> & list ,int left, int right, size_t k, int curDim);
    KDTree<Dim>::KDTreeNode* KDTree_helper(vector<Point<Dim>>& newPoints, int left, int right, int curDim);
    void copy_helper(KDTreeNode * cur, KDTree* other);
    void delete_helper(KDTreeNode* cur);

    void find_helper(KDTreeNode* subroot, Point<Dim> & currentBest, Point<Dim> &target, int curDim)const;
