...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    map<string, vector<string>>::const_iterator lookup1 = dict.find(one);
    map<string, vector<string>>::const_iterator lookup2 = dict.find(two);

    if(lookup1 == dict.end() || lookup2 == dict.end()){
        return false;
    }

    if(lookup1->second.size() != lookup2->second.size()){
        return false;
    }

    for(unsigned int i =0; i< lookup1->second.size(); i++){
            // std::cout<<lookup1->second[i];
            // std::cout<<lookup2->second[i]; 
        if(lookup1->second[i] != lookup2->second[i]){
            return false;       
        }
