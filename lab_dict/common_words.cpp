...
void CommonWords::init_common()
{
    ...
    for(size_t i = 0; i < file_word_maps.size() ; i++){
      for(map<string, unsigned int>::iterator j = file_word_maps[i].begin(); j != file_word_maps[i].end() ; ++j){
        map<string, unsigned int>::iterator lookup = common.find(j->first);
...
vector<string> CommonWords::get_common_words(unsigned int n) const
{
    ...
    for(map<string, unsigned int>::const_iterator it = common.begin(); it != common.end(); ++it){
        if(it->second == file_word_maps.size()){
            check = true;
            for(size_t i = 0; i < file_word_maps.size(); i++){
            map<string, unsigned int>::const_iterator lookup = file_word_maps[i].find(it->first);
