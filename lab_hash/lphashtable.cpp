...
void LPHashTable<K, V>::remove(K const& key)
{
    ...
    //std::cout<<"check remove"<<std::endl;
...
int LPHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
            //std::cout<<"check findindex"<<std::endl;
...
void LPHashTable<K, V>::resizeTable()
{
     ...
     */
     ...
     //tmp with new size
    std::pair<K, V>** tmp_table = new std::pair<K, V>*[new_size];
    bool* tmp_probe = new bool[new_size];
    ...
    for(size_t i=0; i<size; i++){
        if(table[i] != NULL){
            size_t index = hashes::hash(table[i]->first, new_size);
            while(1){
                if(tmp_table[index] == NULL){
                    tmp_table[index] = table[i];
                    tmp_probe[index] = should_probe[i];
                    break;
                }else{
